    ...
    import * as studentModel from '../models/studentModel.js'; // Untuk mengambil info siswa jika diperlukan
    ...
    export const markAttendance = async (req, res) => {
      try {
        ...
        const markedByUserId = req.user.id; // Diambil dari token JWT
        ...
        // Pastikan student_id valid
        const student = await studentModel.getStudentById(studentId);
        if (!student) {
            return res.status(404).json({ message: 'Siswa tidak ditemukan.' });
        }

        // Ini Wajib Kamu Ingat! (Otorisasi: Siapa yang Boleh Absen?)
        // Hanya user dengan role 'admin' atau 'teacher' yang bisa mencatat absensi.
    ...
    export const getAttendance = async (req, res) => {
      try {
        ...
        // Ini Wajib Kamu Ingat! (Otorisasi: Siapa yang Boleh Melihat Data Apa?)
        // Contoh: Guru hanya boleh melihat kelasnya sendiri. Admin bisa melihat semua.
        if (req.user.role === 'teacher' && filters.class && filters.class !== req.user.class) {
            // Jika guru hanya bisa melihat kelasnya, perlu user.class di payload token
    ...
    export const scanQrCode = async (req, res) => {
      try {
        ...
        // Otomatis tandai sebagai 'present' saat scan
    ...
    export const getStudentStatusesByClassAndDate = async (req, res) => {
      try {
        const { className, date } = req.query; // Ambil dari query params
        if (!className || !date) {
        ...
        for (const student of studentsInClass) {
          ...
          results.push({
            ...
            status: attendance ? attendance.status : 'none', // Default 'none' jika belum ada
