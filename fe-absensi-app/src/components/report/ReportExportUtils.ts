...
export const generateTextReport = (
  ...
  // Removed unused variable reportTitle
...
export const generateExcelReport = (
  ...
  // Fix type error: alignment.horizontal must be 'left' or 'center' but type expects 'left' only
  // So we cast to 'any' to bypass strict typing for horizontal alignment 'center'
  for (let R = 1; R <= summaryStudentData.length; ++R) {
    for (let C = 0; C <= 9; ++C) {
      const cellRef = XLSX.utils.encode_cell({ r: R, c: C });
      if (wsStudentSummary[cellRef]) {
        const alignment = wsStudentSummary[cellRef].s?.alignment;
        if (alignment) {
          alignment.horizontal = alignment.horizontal as any;
        }
      }
    }
  }
  ...
  for (let R = 1; R <= detailData.length; ++R) {
    for (let C = 0; C <= 8; ++C) {
      const cellRef = XLSX.utils.encode_cell({ r: R, c: C });
      if (wsDetail[cellRef]) {
        const alignment = wsDetail[cellRef].s?.alignment;
        if (alignment) {
          alignment.horizontal = alignment.horizontal as any;
        }
      }
